...
password_gen(username, password)                                          insert generated password
...
def password_gen(username, password):
    db = MySQLdb.connect("localhost", "root", "asoaso7676", "site_ap", use_unicode=True, charset="utf8")
    cursor = db.cursor()
...
def put_link(id_link, link, link_name='link'):
    db = MySQLdb.connect("localhost", "root", "asoaso7676", "site_ap", use_unicode=True, charset="utf8")
    cursor = db.cursor()
    sql = "INSERT INTO links VALUES ('%d', '%s' , '%s')" % (id_link, link, link_name)
    flag = False
    try:
        cursor.execute(sql)
        db.commit()
...
def get_link(id_link):
    db = MySQLdb.connect("localhost", "root", "asoaso7676", "site_ap", use_unicode=True, charset="utf8")
    cursor = db.cursor()
    sql = "SELECT * FROM links WHERE id_link = '%d'" % id_link
    try:
        cursor.execute(sql)
        data = cursor.fetchall()
        db.commit()
...
#print put_link(14, "acasc",'acsasc')
#a= get_link(14)
#print int(a[0][0])
#print password_gen("c","ascssssssss")
...
#print posting("c", "adfavdva",'advdavdav')
